...
class Checkers:
    ...
    def CompTurn(s):
		...
		#check if moves are contained in allWinningMoves before moving
		if s.movesAvailable() < 10 and s.Action(s.moves[m][2],s.moves[m][3]) in s.allWinningMoves
			#min-max implementation
			for s.Action(s.moves[m][2],s.moves[m][3]) in s.allWinningMoves[s.Action(s.moves[m][2],s.moves[m][3]).index :]:
				s.Action(s.allWinningMoves[m][2],s.allWinningMoves[m][3])
			#check if win and show message
			if s.movesAvailable() == [] and s.numColour(s.pTurn):
				#save the winning sequence of moves to all winning moves
				if s.opposite(s.pTurn) == 'White'
					s.allWinningMoves.append(s.allWhiteMoves)
				else:
					s.allWinningMoves.append(s.allBlackMoves)
		...
		#save move made
		if s.pTurn == 'White'
			s.allWhiteMoves.append(s.Action(s.moves[m][2],s.moves[m][3]))
		else:
			s.allBlackMoves.append(s.Action(s.moves[m][2],s.moves[m][3]))
    ...
    def movesAvailable(s):
        ...
        firstMove = [1, 2, 3, 4, 5, 6, 7, 8]
        
        while firstMove > 0:
        #+- j to select a random number from the possible 8 to remove the lateral iteration process
        j = random.choice(firstMove)
        #+-loop throught the 8 possible forward moves
        for i in range(8):
            X1,Y1 = [i-1,i+1],[j-1,j+1]
            #+-check for the four possible moves around a given square
            for a in range(2):
                for b in range(2):
                    if 0<=X1[a]<8 and 0<=Y1[b]<8:
                        if s.moveIsValid(i,j,X1[a],Y1[b]):
                            ...
                            return moves
                        #+-remove j from the lateral moves to explore before selecting next move in the event of no suitable move found
                        firstMove.remove(j)
                        #+-repeat the process until firstMove is empty
